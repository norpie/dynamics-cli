...
impl AppLauncher {
    fn init(ctx: &mut Context) -> Self {
        ...
        let all_entries = ctx.runtime.get_all_apps()
            .into_iter()
            .map(|(app_id, lifecycle)| {
                let factory = ctx.runtime.get_factory(app_id);
                AppLauncherEntry {
                    app_id,
                    name: factory.name(),
                    description: factory.description(),
                    status: match lifecycle {
                        AppLifecycle::Running => AppStatus::Running,
                        AppLifecycle::Background => AppStatus::Background,
                        AppLifecycle::NotCreated | AppLifecycle::Dead => AppStatus::NotStarted,
                    },
                }
            })
            .collect();
...
**Apps register themselves in Runtime::new()** - same as V1:
...
impl Runtime {
    pub fn new() -> Self {
        ...
        // Register all apps
        factories.insert(AppId::MigrationEnvironment, MigrationEnvironmentFactory);
        factories.insert(AppId::EntityComparison, EntityComparisonFactory);
        factories.insert(AppId::Settings, SettingsFactory);
        // ... all other apps

        // Launcher discovers apps by iterating factories
        Self { factories, /* ... */ }
    }
}
```

**Each factory provides metadata:**
```rust
trait AppFactory {
    fn name(&self) -> &'static str;
    fn description(&self) -> &'static str;
    fn create(&self, params: Box<dyn Any>) -> Box<dyn App>;
    // ... quit_policy, suspend_policy, etc.
}

// Example
impl AppFactory for MigrationEnvironmentFactory {
    fn name(&self) -> &'static str {
        "Migration Environments"
    }

    fn description(&self) -> &'static str {
        "Manage Dynamics 365 migration environments and comparisons"
    }
...
**Launcher queries runtime** for all registered factories to build the list.
...
**V2 launcher:**
- Auto-discovers from runtime factories
- Shows app lifecycle status
- Built-in search
- Sorted by recency + status

**Code changes:**
- Add `name()` and `description()` to AppFactory trait
- Add `get_all_apps()` and `get_factory()` methods to Runtime
- Launcher queries runtime on open instead of hardcoding list
