...
pub struct AppLauncherEntry {
    pub app_id: String,      // String ID (e.g., "migration_environments")
    pub name: String,
...
```rust
fn sort_app_entries(
    entries: &mut [AppLauncherEntry],
    last_active_time: &HashMap<String, Instant>,
) {
    entries.sort_by(|a, b| {
        use std::cmp::Ordering;

        // First: Sort by status (Running > Background > NotStarted)
        match (a.status, b.status) {
            (AppStatus::Running, AppStatus::Running) => {
                // Both running: sort by recency
                let a_time = last_active_time.get(&a.app_id);
                let b_time = last_active_time.get(&b.app_id);
                b_time.cmp(&a_time)  // Most recent first
            }
            (AppStatus::Running, _) => Ordering::Less,  // Running comes first
            (_, AppStatus::Running) => Ordering::Greater,

            (AppStatus::Background, AppStatus::Background) => {
                // Both background: sort by recency
                let a_time = last_active_time.get(&a.app_id);
                let b_time = last_active_time.get(&b.app_id);
                b_time.cmp(&a_time)  // Most recent first
            }
            (AppStatus::Background, _) => Ordering::Less,  // Background before NotStarted
            (_, AppStatus::Background) => Ordering::Greater,

            (AppStatus::NotStarted, AppStatus::NotStarted) => {
                // Both not started: alphabetical by name
                a.name.cmp(&b.name)
            }
        }
    });
...
**No fuzzy matching** - keep it simple. Users can type a few characters from the name.
...
impl AppLauncher {
    fn init(ctx: &mut Context) -> Self {
        // Query runtime for all registered apps and their statuses
        let mut all_entries: Vec<AppLauncherEntry> =
            inventory::iter::<AppRegistration>()
                .map(|reg| {
                    let status = if ctx.runtime.active_app == reg.id {
                        AppStatus::Running
                    } else if ctx.runtime.background_apps.contains_key(reg.id) {
        ...
        // Sort by status + recency
        sort_app_entries(&mut all_entries, &ctx.runtime.last_active_time);
    ...
    fn on_search_changed(&mut self, query: String) {
        self.search_query = query;
        self.filtered_entries = filter_entries(&self.all_entries, &self.search_query);
        self.list_state.select(0);  // Reset to first item
    }

    fn on_select(&mut self, ctx: &mut Context, index: usize) {
        if let Some(entry) = self.filtered_entries.get(index) {
            ctx.navigate_to(&entry.app_id);  // String ID
        }
    }
}
```
...
**List item rendering:**
```rust
impl ListItem for AppLauncherEntry {
    fn render(&self, selected: bool, theme: &Theme) -> Element {
        ...
        let status_text = match self.status {
            ...
            AppStatus::Background => "[Background]",
            AppStatus::NotStarted => "",
        };

        let status_color = match self.status {
            AppStatus::Running => theme.focus,
            AppStatus::Background => theme.text_dim,
            AppStatus::NotStarted => theme.text_muted,
        };

        let bg = if selected { theme.selection } else { theme.bg_base };

        col![
            row![
                text(format!("{} {}", status_icon, self.name))
...
**Apps auto-register using the `inventory` crate** - no manual registration needed:
...
**Each app file has ONE inventory submission** - this is the ONLY place to register:
...
inventory::submit! {
    AppRegistration {
        ...
        description: "Manage Dynamics 365 migration environments and comparisons",
        constructor: |ctx| Box::new(MigrationEnvironmentApp::new(ctx)),
        lifecycle: Lifecycle::Background,
    }
}

// At the bottom of settings_app.rs
inventory::submit! {
    AppRegistration {
        id: "settings",
        name: "Settings",
        description: "Configure application options",
        constructor: |ctx| Box::new(SettingsApp::new(ctx)),
        lifecycle: Lifecycle::Destroy,
    }
}

// At the bottom of operation_queue_app.rs
inventory::submit! {
    AppRegistration {
        id: "operation_queue",
        name: "Operation Queue",
        description: "Manage and execute API operation batches",
        constructor: |ctx| Box::new(OperationQueueApp::new(ctx)),
        lifecycle: Lifecycle::Background,
    }
}
...
impl Runtime {
    pub fn new() -> Self {
        // Inventory collects all submissions automatically
        let registrations: HashMap<String, &'static AppRegistration> =
            inventory::iter::<AppRegistration>()
                .map(|reg| (reg.id.to_string(), reg))
                .collect();

        log::info!("Discovered {} apps", registrations.len());
    ...
    pub fn navigate_to(&mut self, app_id: &str) {
        let reg = self.registrations.get(app_id)
            .expect("Unknown app ID");

        // Handle current app lifecycle...
...
**Benefits:**
- ✅ **Zero boilerplate** - One `inventory::submit!` per app, nothing else
- ✅ **No enum maintenance** - String IDs, no AppId enum to update
- ✅ **No trait impls** - No factory traits to implement
- ✅ **Compile-time discovery** - inventory collects at build time
- ✅ **Type-safe** - Compile fails if constructor returns wrong type
- ✅ **Auto-discovery** - Launcher sees all apps automatically
...
**When launcher modal is open:**
- **↑/↓** - Navigate list (handled by list component)
- **Enter** - Launch selected app
- **Esc** - Close launcher (return to previous app)
- **Any typing** - Focus search input and start filtering

**Global keybind** (always available):
- **Ctrl+Space** - Open launcher from any app

#### Benefits

✅ **Simple** - No complex categorization or grouping
✅ **Discoverable** - All apps shown with descriptions
✅ **Status-aware** - See what's running vs backgrounded
✅ **Searchable** - Quickly find apps by name/description
✅ **Recency-based** - Most recently used apps bubble to top
✅ **Fast to implement** - Reuses existing list component + text input
✅ **No maintenance** - Apps auto-appear when registered in Runtime

#### Migration from V1

**V1 launcher:**
- Hardcoded app list in launcher
- AppId enum must be updated for each new app
- Factory trait implementations required
- Manual registration in Runtime::new()
- No status indicators
- No search

**V2 launcher:**
- Auto-discovers via inventory crate
- String IDs, no enum to maintain
- Single `inventory::submit!` per app
- Shows app lifecycle status
- Built-in search
- Sorted by recency + status

**Code changes:**
- Replace `AppId` enum with string IDs throughout
- Remove AppFactory trait and implementations
- Add `inventory::submit!` to each app file
- Launcher uses `inventory::iter::<AppRegistration>()` to build list
- Runtime uses `HashMap<String, &'static AppRegistration>`
...
**Apps return different keybind sets based on state** - the help menu displays whatever the app returned from its most recent `keybinds()` call. This means:
...
impl Runtime {
    fn generate_help(&self) -> Vec<HelpEntry> {
        ...
        // 3. App keybinds (state-dependent via keybinds() method)
        entries.extend(self.generate_app_help());

        // 4. Focused component keybinds (if any component focused)
        if let Some(focused) = self.focused_component() {
    ...
    fn generate_global_help(&self) -> Vec<HelpEntry> {
        let registry = &self.config.options.registry();
        let mut entries = vec![];

        // Find all global.* keybinds (excluding global.nav.*)
        for option_def in registry.list_namespace("keybind") {
            let key = &option_def.key;
            if !key.starts_with("keybind.global.") || key.starts_with("keybind.global.nav.") {
                continue;
            }

            // Skip aliases - we'll collect them in the primary entry
            if key.contains(".alias") {
                continue;
            }

            let action_key = key.strip_prefix("keybind.").unwrap();
            let keys = self.collect_all_bindings(action_key);

            entries.push(HelpEntry {
                keys,
                description: option_def.description.clone(),
                category: HelpCategory::Global,
            });
        }
    ...
    fn generate_navigation_help(&self) -> Vec<HelpEntry> {
        let registry = &self.config.options.registry();
        let mut entries = vec![];

        // Find all global.nav.* keybinds
        for option_def in registry.list_namespace("keybind") {
            let key = &option_def.key;
            if !key.starts_with("keybind.global.nav.") {
                continue;
            }
    ...
    fn generate_app_help(&self) -> Vec<HelpEntry> {
        let mut entries = vec![];

        // Call app's keybinds() method to get current state's bindings
        let app_keybinds = self.active_app.keybinds(&self.ctx);

        for keybind in app_keybinds {
            // Each Keybind has keys (with aliases already included) and description
            entries.push(HelpEntry {
                keys: keybind.keys,
                description: keybind.description,
                category: HelpCategory::App,
            });
        }
    ...
    fn generate_component_help(&self, component: &dyn Component) -> Vec<HelpEntry> {
        // Get which navigation actions this component handles
        let handled_actions = component.handled_nav_actions();

        let mut entries = vec![];
        for action in handled_actions {
            let action_key = format!("global.nav.{}", action.to_key());
            let keys = self.collect_all_bindings(&action_key);
            ...
            entries.push(HelpEntry {
                keys,
                description,
                category: HelpCategory::Component,
            });
        }
    ...
    /// Collect all bindings (primary + all aliases) for a given action key
...
impl ListComponent {
    fn handled_nav_actions(&self) -> Vec<NavActionInfo> {
        vec![
            ...
            NavActionInfo {
                action: NavAction::PageDown,
                description: "Scroll down one page".to_string()
            },
            NavActionInfo {
            ...
            NavActionInfo {
                ...
                description: "Activate selected item".to_string()
            },
        ]
    }
}
...
impl TreeComponent {
    fn handled_nav_actions(&self) -> Vec<NavActionInfo> {
        vec![
            NavActionInfo {
                action: NavAction::Up,
                description: "Navigate up in tree".to_string()
            },
            NavActionInfo {
                action: NavAction::Down,
                description: "Navigate down in tree".to_string()
            },
            NavActionInfo {
                action: NavAction::Left,
                description: "Collapse node or go to parent".to_string()
            },
            NavActionInfo {
                action: NavAction::Right,
                description: "Expand node or enter child".to_string()
            },
            NavActionInfo {
                action: NavAction::Activate,
                description: "Toggle node expansion".to_string()
            },
        ]
    }
}
...
impl TextInputComponent {
    fn handled_nav_actions(&self) -> Vec<NavActionInfo> {
        vec![
            ...
            NavActionInfo {
                action: NavAction::Home,
                description: "Jump to start of input".to_string()
            },
            NavActionInfo {
                action: NavAction::End,
                description: "Jump to end of input".to_string()
            },
            NavActionInfo {
                action: NavAction::Activate,
...
fn render_help_modal(entries: &[HelpEntry], theme: &Theme) -> Layer {
    Layer::centered(80, 40, panel("Help - Keyboard Shortcuts", |ui| {
        ...
        // Current view section (app keybinds)
        if app.clone().count() > 0 {
            ui.text("Current View")
                .style(theme.text_primary.bold());
            ui.spacer(1);
        ...
        // Focused component section (separate from others)
        if component.clone().count() > 0 {
            ui.text("Focused Component")
                .style(theme.text_primary.bold());
            ui.spacer(1);
...
**Modal showing (different app keybinds):**
...
```rust
// Type-safe publishing - automatically serializes
...
```rust
// From any app - send typed work to a queue
...
impl App for OperationQueue {
    fn update(&mut self, ctx: &mut Context) -> Vec<Layer> {
        // Process items from queue at own pace
...
pub enum Progress {
    /// No progress information available (shows spinner, no percentage)
    Indeterminate,

    /// Step-based progress - X of Y units completed
    /// Units could be items, bytes, records, API calls, etc.
    Steps {
        current: u64,
        total: u64,
        label: Option<String>,
    ...
    /// Percentage-based progress (0.0 - 1.0)
    /// Use when you know percentage but not absolute counts
    Percentage {
        value: f32,  // 0.0 = 0%, 1.0 = 100%
        label: Option<String>,
    },

    /// Multi-phase operation - current phase with optional sub-progress
    /// Maximum 1 level of nesting (phase_progress cannot contain another Phase)
    Phase {
        current: usize,   // Current phase (0-indexed)
        total: usize,     // Total number of phases
        name: String,     // Current phase name ("Downloading", "Parsing", "Validating")
        phase_progress: Option<Box<Progress>>,  // Progress within current phase
    },

    /// Time elapsed with no known end time (shows elapsed time only)
    Elapsed {
        elapsed: std::time::Duration,
    },

    /// Time-based progress (for operations with known duration)
    Duration {
        elapsed: std::time::Duration,
...
pub enum Resource<T, E = String> {
    NotAsked,
    Loading(Progress),  // Always has Progress, use Indeterminate for unknown
    Success(T),
    Failure(E),
}
```

**Change from V1:** `Loading` now always contains a `Progress`. Use `Progress::Indeterminate` for V1 behavior.

#### Helper Methods

```rust
impl<T, E> Resource<T, E> {
    /// Create loading state with indeterminate progress (V1 behavior)
    pub fn loading() -> Self {
        Resource::Loading(Progress::Indeterminate)
    }

    /// Create loading state with step progress
    pub fn loading_steps(current: u64, total: u64) -> Self {
        Resource::Loading(Progress::Steps {
            current,
            total,
            label: None
        })
    ...
    /// Create loading state with custom label
    pub fn loading_steps_labeled(current: u64, total: u64, label: String) -> Self {
        Resource::Loading(Progress::Steps {
            current,
            total,
            label: Some(label)
    ...
    /// Create loading state with percentage
    pub fn loading_percentage(value: f32) -> Self {
        Resource::Loading(Progress::Percentage {
            value,
            label: None
        })
    }

    /// Create loading state showing elapsed time only
    pub fn loading_elapsed(elapsed: std::time::Duration) -> Self {
        Resource::Loading(Progress::Elapsed { elapsed })
    }

    /// Create loading state with duration progress
    pub fn loading_duration(elapsed: std::time::Duration, total: std::time::Duration) -> Self {
        Resource::Loading(Progress::Duration { elapsed, total })
    }

    /// Update progress if currently loading
    pub fn update_progress(&mut self, progress: Progress) {
        if let Resource::Loading(_) = self {
            *self = Resource::Loading(progress);
        }
    }

    /// Get current progress if loading
    pub fn progress(&self) -> Option<&Progress> {
        match self {
            Resource::Loading(progress) => Some(progress),
            _ => None,
        }
    }
}
...
impl Progress {
    /// Get progress as a percentage (0.0 - 1.0), if determinable
    pub fn as_percentage(&self) -> Option<f32> {
        match self {
            Progress::Indeterminate | Progress::Elapsed { .. } => None,
            Progress::Steps { current, total, .. } => {
                Some((*current as f32 / *total as f32).clamp(0.0, 1.0))
            }
            Progress::Percentage { value, .. } => Some(value.clamp(0.0, 1.0)),
            Progress::Phase { phase_progress, .. } => {
                phase_progress.as_ref().and_then(|p| p.as_percentage())
            }
            Progress::Duration { elapsed, total } => {
    ...
    pub fn display(&self) -> String {
        match self {
            Progress::Indeterminate => "Loading...".to_string(),
            Progress::Steps { current, total, label } => {
                label.clone().unwrap_or_else(|| format!("{} / {}", current, total))
            }
            Progress::Percentage { value, label } => {
                label.clone().unwrap_or_else(|| format!("{:.0}%", value * 100.0))
            }
            Progress::Phase { current, total, name, phase_progress } => {
                let base = format!("{} ({}/{})", name, current + 1, total);
                if let Some(sub) = phase_progress {
                    if let Some(pct) = sub.as_percentage() {
                        format!("{} - {:.0}%", base, pct * 100.0)
                    } else {
                        base
                    }
                } else {
            ...
            Progress::Duration { elapsed, total } => {
                format!("{:.1}s / {:.1}s", elapsed.as_secs_f32(), total.as_secs_f32())
            }
        }
    }

    /// Validate nesting depth (Phase cannot contain another Phase)
    pub fn validate_nesting(&self) -> Result<(), &'static str> {
        match self {
            Progress::Phase { phase_progress: Some(sub), .. } => {
                match **sub {
                    Progress::Phase { .. } => Err("Phase cannot be nested inside Phase"),
                    _ => Ok(())
                }
            }
            _ => Ok(())
...
**Step-based progress (batch operations):**
```rust
// Preload 20 entity comparisons
state.comparisons = Resource::loading();
...
**Percentage progress:**
```rust
// When you know percentage but not absolute counts
state.sync = Resource::Loading(Progress::Percentage {
    value: 0.45,  // 45%
    label: Some("Syncing data...".to_string()),
});
```

**Elapsed time (unknown duration):**
```rust
// Show how long operation has been running
let start = Instant::now();
loop {
    state.operation = Resource::Loading(Progress::Elapsed {
        elapsed: start.elapsed(),
    });

    if let Some(result) = check_completion().await {
        state.operation = Resource::Success(result);
        break;
    }
}
```

**Duration progress (known timeout):**
```rust
// Operation with known time limit
let start = Instant::now();
let timeout = Duration::from_secs(30);

loop {
    let elapsed = start.elapsed();
    state.operation = Resource::Loading(Progress::Duration {
        elapsed,
        total: timeout,
    });

    if elapsed >= timeout {
        state.operation = Resource::Failure("Timeout".to_string());
        break;
    }

    if let Some(result) = try_operation().await {
        state.operation = Resource::Success(result);
        break;
    }
}
...
state.file = Resource::Loading(Progress::Phase {
    ...
    phase_progress: Some(Box::new(Progress::Steps {
        ...
        label: Some(format!("{:.1} MB / {:.1} MB",
            ...
            total_bytes as f32 / 1_000_000.0)),
    })),
});

// Phase 2: Parsing (indeterminate within phase)
state.file = Resource::Loading(Progress::Phase {
    current: 1,
    total: 3,
...
state.file = Resource::Loading(Progress::Phase {
    ...
    phase_progress: Some(Box::new(Progress::Percentage {
        value: validation_progress,
        label: None,
    })),
});

state.file = Resource::Success(parsed_file);
```

#### Updating Progress from Async Tasks

**Pattern 1: Channel-based updates:**
```rust
// Start async task with progress channel
...
// In update(), receive progress updates
fn update(&mut self, ctx: &mut Context) -> Vec<Layer> {
...
**Pattern 2: Shared state updates:**
```rust
let data = Arc::new(Mutex::new(Resource::loading()));
...
ctx.spawn(async move {
    for (i, item) in items.iter().enumerate() {
        ...
        *data_clone.lock().unwrap() = Resource::Loading(Progress::Steps {
            current: i as u64,
            total: items.len() as u64,
            label: None,
        });
    ...
    *data_clone.lock().unwrap() = Resource::Success(all_results);
});
```
        ...
        Layer::fill(panel("Data", |ui| {
            match &self.data {
                Resource::Loading(progress) => {
                    // Framework provides default rendering
                    match progress {
                        ...
                        prog => {
                            ui.spinner(prog.display());
                        }
                    }
                }
                Resource::Success(data) => {
                    ui.text(format!("Loaded {} items", data.len()));
                }
                ...
                Resource::NotAsked => {
                    ui.button("Load Data").on_click(Self::handle_load);
                }
            }
        }))
    ]
...
match &self.data {
    Resource::Loading(progress) => {
        ui.col(|ui| {
            ui.text(progress.display());

            if let Some(pct) = progress.as_percentage() {
                // Custom progress bar
                ui.progress_bar(pct)
                    .height(2)
                    .style(theme.accent_success);
            } else {
                // Custom spinner
                ui.spinner_dots();
            }
...
enum Resource<T, E = String> {
    NotAsked,
    Loading,  // No progress information
    Success(T),
    Failure(E),
}
```
...
// To get V1 behavior:
Resource::loading()  // Same as Loading(Progress::Indeterminate)
```
...
pub enum ErrorKind {
    /// Temporary network/infrastructure issue
    Transient,
    ...
    /// Unrecoverable error (disk full, invalid state, etc.)
    Fatal,
}
...
pub enum RetryStrategy {
    ...
    /// Exponential backoff starting with base delay
    /// Delays: base, base*2, base*4, base*8, ...
    Exponential {
        base: Duration,
        max_delay: Duration,
...
```rust
impl ResourceError {
    /// Create a transient error (network timeout, etc.)
    ...
    pub fn rate_limit(message: impl Into<String>, retry_after: Duration) -> Self {
        Self {
            ...
            retry_strategy: RetryStrategy::After(retry_after),
            context: HashMap::new(),
        }
    }
    ...
    pub fn authentication(message: impl Into<String>) -> Self {
        Self {
            ...
            retry_strategy: RetryStrategy::None,  // Requires user action
            context: HashMap::new(),
        }
    }
    ...
    pub fn validation(message: impl Into<String>) -> Self {
        Self {
            ...
            retry_strategy: RetryStrategy::None,  // Requires user to fix input
            context: HashMap::new(),
        }
    }
    ...
    pub fn not_found(message: impl Into<String>) -> Self {
        Self {
            ...
            retry_strategy: RetryStrategy::None,  // Won't help to retry
            context: HashMap::new(),
        }
    }
    ...
    pub fn permission(message: impl Into<String>) -> Self {
        Self {
            ...
            retry_strategy: RetryStrategy::None,  // Requires permission change
            context: HashMap::new(),
        }
    }
    ...
    pub fn is_retryable(&self) -> bool {
        !matches!(self.retry_strategy, RetryStrategy::None)
    }
    ...
    pub fn next_retry_delay(&self, retry_count: usize) -> Option<Duration> {
        match &self.retry_strategy {
            ...
            RetryStrategy::After(delay) => Some(*delay),
            RetryStrategy::Exponential { base, max_delay, max_attempts } => {
...
```rust
// Plain string errors default to fatal
impl From<String> for ResourceError {
    fn from(message: String) -> Self {
        ResourceError::fatal(message)
    }
}

impl From<&str> for ResourceError {
    fn from(message: &str) -> Self {
        ResourceError::fatal(message.to_string())
    }
}

// std::io::Error - smart mapping based on error kind
impl From<std::io::Error> for ResourceError {
    fn from(err: std::io::Error) -> Self {
        match err.kind() {
            std::io::ErrorKind::NotFound => ResourceError::not_found(err.to_string()),
            std::io::ErrorKind::PermissionDenied => ResourceError::permission(err.to_string()),
            std::io::ErrorKind::ConnectionRefused
            | std::io::ErrorKind::ConnectionReset
            | std::io::ErrorKind::TimedOut => ResourceError::transient(err.to_string()),
            _ => ResourceError::fatal(err.to_string()),
        }
    }
}

// reqwest::Error - smart mapping based on status codes
impl From<reqwest::Error> for ResourceError {
    fn from(err: reqwest::Error) -> Self {
        if err.is_timeout() || err.is_connect() {
            return ResourceError::transient(err.to_string());
        }

        if let Some(status) = err.status() {
            match status.as_u16() {
                401 | 403 => return ResourceError::authentication(err.to_string()),
                404 => return ResourceError::not_found(err.to_string()),
                429 => {
                    // Parse Retry-After header if available
                    let retry_after = Duration::from_secs(60);  // Default
                    return ResourceError::rate_limit(err.to_string(), retry_after);
                }
                500..=599 => return ResourceError::transient(err.to_string()),
                _ => {}
            }
        }

        ResourceError::fatal(err.to_string())
    }
}

// anyhow::Error - try to downcast to known types
...
```rust
impl<T> Resource<T> {
    /// Create a failure with retry count
    pub fn failure(error: impl Into<ResourceError>) -> Self {
        Resource::Failure {
            error: error.into(),
            retry_count: 0,
        }
    ...
    pub fn is_failure(&self) -> bool {
        matches!(self, Resource::Failure { .. })
    }

    /// Get error if failed
    pub fn error(&self) -> Option<&ResourceError> {
        match self {
            Resource::Failure { error, .. } => Some(error),
            _ => None,
        }
    }
    ...
    pub fn retry_count(&self) -> usize {
        match self {
            Resource::Failure { retry_count, .. } => *retry_count,
            _ => 0,
        }
    }
    ...
    pub fn is_retryable(&self) -> bool {
        match self {
            Resource::Failure { error, .. } => error.is_retryable(),
            _ => false,
        }
    }
    ...
    pub fn next_retry_delay(&self) -> Option<Duration> {
        match self {
            Resource::Failure { error, retry_count } => {
                error.next_retry_delay(*retry_count)
            }
            _ => None,
        }
    }
    ...
    pub fn increment_retry(&mut self) {
        if let Resource::Failure { retry_count, .. } = self {
            *retry_count += 1;
        }
    }
}
...
**Simple error handling with automatic conversion:**
```rust
async fn fetch_data() -> Result<Data, ResourceError> {
    // reqwest::Error automatically converts to ResourceError
    let response = reqwest::get("https://api.example.com/data").await?;

    if response.status() == 404 {
        return Err(ResourceError::not_found("Data not found"));
    }
...
match fetch_data().await {
    ...
    Err(err) => state.data = Resource::failure(err),
}
```

**Custom error with context:**
```rust
let result = fetch_entity(&entity_id).await
    .map_err(|e| {
        ResourceError::transient(format!("Failed to fetch entity: {}", e))
            .with_context("entity_id", entity_id)
            .with_context("timestamp", Utc::now().to_string())
    });

match result {
    Ok(entity) => state.entity = Resource::Success(entity),
    Err(err) => state.entity = Resource::failure(err),
}
```
...
```rust
// Framework helper for automatic retry
impl Context {
    /// Spawn task with automatic retry on transient errors
    pub fn spawn_with_retry<T, F, Fut>(
        &mut self,
        resource: &mut Resource<T>,
        operation: F,
    ) where
        F: Fn() -> Fut + Send + 'static,
        Fut: Future<Output = Result<T, ResourceError>> + Send,
    {
        self.spawn(async move {
            ...
            loop {
                match operation().await {
                    Ok(data) => return Resource::Success(data),
                    Err(err) => {
                        if let Some(delay) = err.next_retry_delay(retry_count) {
                            tokio::time::sleep(delay).await;
                            retry_count += 1;
                        } else {
                            return Resource::Failure { error: err, retry_count };
                        }
                    }
                }
            }
...
if let Resource::Failure { error, .. } = &self.data {
    if error.kind == ErrorKind::RateLimit {
        if let Some(delay) = error.next_retry_delay(self.data.retry_count()) {
            // Start timer to auto-retry
            ctx.timer(delay, Msg::AutoRetry);
        }
    }
}

// Handle auto-retry message
Msg::AutoRetry => {
    self.data.increment_retry();
    ctx.spawn_into(&mut self.data, async {
        fetch_data().await
            ...
            if error.is_retryable() {
                ...
                if let Some(delay) = error.next_retry_delay(*retry_count) {
                    ...
                    ui.text("Max retry attempts exceeded")
                        .style(theme.warning);
                    ui.button("Try Again")
                        .on_click(Self::handle_reset_and_retry);
                }
            } else {
                ...
                match error.kind {
                    ...
                    ErrorKind::Authentication => {
                        ui.button("Re-authenticate")
                            .on_click(Self::handle_reauth);
                    }
                    ErrorKind::Permission => {
                        ui.text("Insufficient permissions. Contact administrator.")
                            .style(theme.warning);
                    }
                    _ => {
...
// Set error - automatic conversion from String
state.data = Resource::failure("Network timeout");  // Defaults to Fatal
...
state.data = Resource::failure(reqwest_error);  // Automatic ErrorKind detection
```
...
(All major design items completed)
