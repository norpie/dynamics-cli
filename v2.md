...
**Needs design before macros can be finalized:**

- **Layout primitives**: How do rows/columns actually work in immediate mode?
- **Constraint system**: Fill, Length, Min - how are these applied during rendering?
- **Nesting behavior**: How do nested layouts compute their dimensions?
- **Builder API**: What does `ui.row()` / `ui.col()` look like?

**Macros (deferred until layout system designed):**
- `col!`, `row!`, `spacer!`, `use_constraints!()` - likely still useful, but syntax may change
- All other macros (button_row!, modal!, form_layout!, etc.) - binned
...
### Multi-Runtime: App Registration & Lifecycle

#### Zero-Boilerplate Registration

**Problem in V1:** Registering an app requires editing 3 files:
1. Add `AppId` enum variant
2. Import app type in `multi_runtime.rs`
3. Call `factories.insert(AppId::MyApp, ...)`

**V2 Solution:** Apps register themselves using `inventory` crate.

```rust
// apps/my_app.rs - ONE location only
pub struct MyApp {
    // ... state
}
...
impl App for MyApp {
    fn id() -> &'static str {
        "my-app"  // Unique identifier
    }

    fn title() -> &'static str {
        "My Application"  // For launcher UI
    }

    fn update(&mut self, ctx: &mut Context) -> Vec<Layer> {
        vec![
            Layer::fill(panel("My App", |ui| {
                ui.button("Go to Other App")
                    .on_click(Self::handle_navigate);
            }))
        ]
    }

    fn handle_navigate(&mut self, ctx: &mut Context) {
        ctx.navigate(OtherApp::id());  // Type-safe, autocomplete!
    }

    // ... other trait methods
}

// Self-registration - happens at module load time
inventory::submit! {
    AppDescriptor {
        id: MyApp::id(),
        factory: |params| Box::new(MyApp::new(params)),
        title: MyApp::title(),
...
**Runtime discovers apps automatically:**
...
impl MultiAppRuntime {
    pub fn new() -> Self {
        ...
        for descriptor in inventory::iter::<AppDescriptor>() {
            apps.insert(
                descriptor.id.to_string(),
                AppInstance {
                    factory: descriptor.factory,
                    runtime: None,  // Lazy created
                    lifecycle: Lifecycle::NotCreated,
                }
            );
        }
        ...
        Self {
            apps,
            active_app: "app-launcher".to_string(),
        }
    }

    fn navigate(&mut self, target: &str) -> Result<()> {
        if !self.apps.contains_key(target) {
            return Err(anyhow!("Unknown app: {}", target));
        }
        // Handle lifecycle...
        self.active_app = target.to_string();
        Ok(())
    }
}
```

**Navigation uses app IDs:**

```rust
// Type-safe navigation with autocomplete
ctx.navigate(MyApp::id());
ctx.navigate(MigrationEnvironmentApp::id());
ctx.navigate(EntityComparisonApp::id());

// App launcher lists all apps
for descriptor in inventory::iter::<AppDescriptor>() {
    ui.button(descriptor.title)
        .on_click(|ctx| ctx.navigate(descriptor.id));
}
```

**Benefits:**
- ✅ One registration location (in the app file itself)
- ✅ No central enum to maintain
- ✅ No manual imports in runtime
- ✅ Type-safe navigation (`MyApp::id()`)
- ✅ Autocomplete works (type name → `::id()`)
- ✅ Compile-time collection (inventory runs at link time)

**Drawbacks:**
- ⚠️ Typos fail at runtime, not compile-time
- ⚠️ Can't exhaustively match on app IDs

**Lifecycle is already designed** - see "Lifecycle & App Management" section for full details (two states: Destroy/Background, four hooks, much simpler than V1).
